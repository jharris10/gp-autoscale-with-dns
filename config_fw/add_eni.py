...
def add_eni_lambda_handler(event, context):
    ...
    if event_type == 'launch' or event_type == 'terminate':
        ...
        privatesubnetID = metadata.split(",")[5]
        rt53Domain = metadata.split(",")[6]
        hostedZoneId = metadata.split(",")[7]
...
def launch_gw(event, message):
   ...
   # eniId = interfaces_dict['NetworkInterfaces'][0]['NetworkInterfaceId']
    ...
    #
    # Addtional NIC code inserted here
    #CreateTrustEni
    err = createEni(privatesubnetID, securityGroups)
    if err == 'false':
        logger.info("Error: Trust Eni creation failed\n")
        terminate('false', message)
        return
    else:
        trusteniId = err
    ...
    #Wait for the ENI to be 'available'
    err = waitEniReady(trusteniId)
    if err == 'false':
        logger.info("ERROR: Failure waiting for ENI to be ready");
        terminate('false', message);
        return

        #Attach the trust network interface to the instance
    err = attachTrustEni(instanceId, trusteniId)
    if err == 'false':
        logger.info("[ERROR]: Failure attaching Trust ENI to instance");
        terminate('false', message);
        return
    else:
        logger.info("[INFO]: Success! Attached Trust ENI to instance");
...
def allocate_and_attach_Trust_eip(Id):
    eip_address_dict = ec2_client.describe_addresses()
    #List of IP addresses is not empty, so we may have an unassociated IP address?
    eipList = eip_address_dict['Addresses']
    if not eipList:
...
def createEniTrust(privatesubnetID, securityGroups):
    try:
        nif = ec2.create_network_interface(SubnetId=privatesubnetID, Groups=[securityGroups], Description="GPGateway Trust")
    except Exception as e:
        logger.info("[ERROR]: ENI private subnet creation failed {}".format(e))
        logger.info(e)
        return 'false'
    else:
        logger.info("INFO: ENI private subnet Created.\n")
        nif.modify_attribute(SourceDestCheck={'Value': False})
        while True:
            try:
                nif.reload()
...
def waitEniReady(Id):
    try:
        ...
        logger.info("[ERROR]: ENI failed to reach desired state {}".format(error))
        return 'false'
    else:
       return 'true'
...
def attachTrustEni(ec2Id, Id):
    try:
        err = ec2_client.attach_network_interface(NetworkInterfaceId=Id, InstanceId=ec2Id,DeviceIndex=2)
    except Exception as e:
            logger.info("[ERROR]: Failed to attach Trust ENI to EC2 instance {}".format(e))
            return 'false'
    else:
        #logger.info("INFO: ENI attached EC2 instance\n")
        ec2_client.modify_network_interface_attribute(NetworkInterfaceId=Id,
